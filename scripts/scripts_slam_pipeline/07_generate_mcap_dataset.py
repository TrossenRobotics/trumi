...
:Steps:
    ...
    2. For every raw video referenced by the plan, decode the required
       ...
       f. JPEG-compress the image.

    3. Write one MCAP file per episode containing time-aligned robot
       state messages and compressed camera images.
...
def main(
    ...
    :param out_fov: Vertical FOV for fisheye rectification (degrees).
        If omitted, images are resized without undistortion.
    ...
    for ipath in input_dirs:
        ...
        for plan_episode in plan:
            ...
            for cam_id, camera in enumerate(cameras):
                ...
                n_frames_expected = (video_end - video_start) // slam_frame_stride
    ...
    # Phase 2: Determine input image size (needed for resize transform)
    ...
    output_dir = pathlib.Path(output).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    total_steps = sum(len(ep["timestamps"]) for ep in all_episodes)
    logger.info(
        "Writing %d episode MCAP files (%d total steps) to %s",
        len(all_episodes),
        total_steps,
        output_dir,
    )

    def write_episode(ep_idx, episode):
        timestamps = episode["timestamps"]
        robot_state = episode["robot_state"]
        video_sources = episode["video_sources"]
        ep_len = len(timestamps)

        episode_path = output_dir / f"episode_{ep_idx:06d}.mcap"
        ...
        with open(episode_path, "wb") as f:
            ...
            writer.add_metadata(
                ...
                data={
                    ...
                    "n_grippers": str(n_grippers),
                    "n_cameras": str(n_cameras),
                    "out_res": json.dumps(list(out_res)),
                },
            )

            # Pre-decode camera frames for this episode
            cam_frames = {}
            for source in video_sources:
                ...
                if not pkl_path.is_file():
                    raise click.ClickException(
                        f"tag_detection.pkl not found: {pkl_path}"
                    )
                ...
                with av.open(video_path) as container:
                    ...
                    for frame_idx, frame in enumerate(container.decode(in_stream)):
                        ...
                        if frame_idx >= frame_end:
                            break
                        if (frame_idx - frame_start) % slam_frame_stride != 0:
                            continue
                ...
                if len(frames) != ep_len:
                    raise RuntimeError(
                        f"Camera {cam_id} decoded {len(frames)} frames, "
                        f"expected {ep_len} for episode {ep_idx}."
                    )
                cam_frames[cam_id] = frames

            # Write messages step by step (time-aligned)
            seq = 0
            ...
            for gripper_id in range(n_grippers):
                ...
                writer.add_message(
                    channel_id=channel_ids[f"robot{gripper_id}_demo_start"],
                    log_time=t0_ns,
                    publish_time=t0_ns,
                    sequence=seq,
                    data=_encode_eef_state(start_pose[:3], start_pose[3:]),
                )
                seq += 1
                writer.add_message(
                    channel_id=channel_ids[f"robot{gripper_id}_demo_end"],
                    log_time=t0_ns,
                    publish_time=t0_ns,
                    sequence=seq,
                    data=_encode_eef_state(end_pose[:3], end_pose[3:]),
                )
                seq += 1
            ...
            for step in range(ep_len):
                ...
                for gripper_id in range(n_grippers):
                    writer.add_message(
                        channel_id=channel_ids[f"robot{gripper_id}_eef"],
                        log_time=t_ns,
                        publish_time=t_ns,
                        sequence=seq,
                        data=_encode_eef_state(
                    ...
                    writer.add_message(
                        channel_id=channel_ids[f"robot{gripper_id}_gripper"],
                        log_time=t_ns,
                        publish_time=t_ns,
                        sequence=seq,
                        data=_encode_gripper_state(
                ...
                for cam_id in range(n_cameras):
                    writer.add_message(
                        channel_id=channel_ids[f"camera{cam_id}"],
                        log_time=t_ns,
                        publish_time=t_ns,
                        sequence=seq,
                        data=_encode_compressed_image(
                            cam_frames[cam_id][step],
                            t_ns=t_ns,
                            frame_id=f"camera{cam_id}",
    ...
    with tqdm(total=len(all_episodes), desc="Episodes") as pbar:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for ep_idx, episode in enumerate(all_episodes):
                ...
                futures.add(executor.submit(write_episode, ep_idx, episode))
